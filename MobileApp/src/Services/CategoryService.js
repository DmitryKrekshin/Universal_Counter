...
export function GetCategories(callback) {
  const query = `select ${TABLE_NAME}.Id, ${TABLE_NAME}.Name, ${TABLE_NAME}.Value, ${TABLE_NAME}.IconName, ${TABLE_NAME}.IconFamily, ${TABLE_NAME}.ColorHEX, MU.Name as MeasurementUnitName
                 from ${TABLE_NAME}
  ...
  context.transaction((tx) => {
    tx.executeSql(query, [], (tx, result) => {
      for (let i = 0; i < result.rows.length; i++) {
        ...
        categories.push({Id: row.Id, Name: row.Name, Value: row.Value, IconName: row.IconName, IconFamily: row.IconFamily, ColorHEX: row.ColorHEX, MeasurementUnitName: row.MeasurementUnitName});
      }
